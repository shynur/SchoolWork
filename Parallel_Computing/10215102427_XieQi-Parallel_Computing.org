...
根据相关文章[fn:: [[https://github.com/shynur][=shynur=]]: [[https://shynur.github.io/CheatSheets/OpenMP.html][简明 OpenMP: C/C++ 并行计算]]]:
...
1. 执行 ~sudo bash -c 'echo 3 > /proc/sys/vm/drop_caches'~ Linux 针对文件系统提供的 buffer.
...
#+BEGIN_EXAMPLE
./
 |__ 10215102427_XieQi-Parallel_Computing.{org,html,pdf}  (one of which is this file)
 |
...
如果程序不是 I​/​O 密集型的, 那么性能瓶颈很可能也在磁盘这边, 则[[经验法则][该经验法则]]也就无法提供可观的收益.
